...
class IngredientAdapter(val c: Context, val alimentsList:MutableList<Aliments>,val idRepas:String,val mListener:onItemClickListener) : RecyclerView.Adapter<IngredientAdapter.UserViewHolder> () {
                    ...
                    R.id.delete ->{
                        AlertDialog.Builder(c)
                            ...
                            .setPositiveButton("Yes"){
                                    dialog,_->
                                val call = retrofitInterface!!.executeDeleteIngredient(idRepas,position.toString())
                                call!!.enqueue(object :Callback<Void?>{
                                    override fun onResponse(call: Call<Void?>, response: Response<Void?>) {
                                        if (response.code()==200){
